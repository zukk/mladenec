...
function BXIBlockAfterSave($arFields)
{
    ...
    if ($obRes = $res->GetNextElement()) {
        ...
        $ar_prop_500_id = $obRes->GetProperty("IMG500");
        $ar_prop_500_id = $ar_prop_500_id["PROPERTY_VALUE_ID"];

        $new_images500 = array();
        $old_images500 = array();
        $new_images_path500 = array();
        /* end ozon  */
        ...
        foreach ($ar_prop["VALUE"] as $key => $img):
            ...
            $new_images500[$i] = CFile::MakeFileArray($new_img_path);
            $new_images500[$i]["name"] = $img_orig_name;
            $new_images_path500[] = $new_img_path;
        ...
        endforeach;
        /* 500 */
        $PROPERTY_CODE = "IMG500";
        $PROPERTY_VALUE = $new_images500;

        foreach ($ar_prop_500_id as $p):
            $PROPERTY_VALUE[$p] = array("name" => "", "type" => "", "tmp_name" => "", "error" => "4", "size" => "0", "del" => "Y");
        endforeach;

        CIBlockElement::SetPropertyValueCode($ELEMENT_ID, $PROPERTY_CODE, $PROPERTY_VALUE);

        foreach ($new_images_path500 as $path):
            @unlink($path);
        endforeach;
