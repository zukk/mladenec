...
function BXIBlockAfterSave($arFields)
{
    ...
    foreach ($value as $index => $id):
        ...
        creatWaterMarkFile($new_img_path);
    ...
    if ($obRes = $res->GetNextElement()) {
        ...
        foreach ($ar_prop["VALUE"] as $key => $img):
            ...
            if ($i == 0):
                ...
                imagecopyresampled($image_p, $image, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
                $new_img_path = tempnam("/tmp", "FOO").".jpg";
                imagejpeg($image_p, $new_img_path, 70);
                $add_array = CFile::MakeFileArray($new_img_path);
                $add_array["name"] =  $img_orig_name;

                $PROPERTY_CODE = "IMG_255";
                $PROPERTY_VALUE = $add_array;
                ...
                imagecopyresampled($image_p, $image, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
                $new_img_path = tempnam("/tmp", "FOO").".jpg";
                imagejpeg($image_p, $new_img_path, 100);

                $add_array = Array('DETAIL_PICTURE'=>CFile::MakeFileArray($new_img_path));
                $add_array["DETAIL_PICTURE"]["name"] =  $img_orig_name;
                $be = new CIBlockElement();
                $be->Update($ELEMENT_ID, $add_array);*/
...
function creatWaterMarkFile($img)
{
