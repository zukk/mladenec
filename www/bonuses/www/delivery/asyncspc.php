...
function OA_Dal_Delivery_getZoneLinkedAdInfos($zoneid)
{
    ...
    $query =
        ...
        . "c.status <= 0 "
        . "AND "
        . "(d.expire_time >= NOW() or d.expire_time is null) "
        . "AND "
        . "(d.activate_time <= NOW() or d.activate_time is null) "
    ;
...
function _convertContextArray($key, $array)
{
    ...
    foreach ($array as $value) {
        ...
        switch ($item) {
            case 'c':
                $unpacked[] = array($key => 'campaignid:' . $id);
                break;
            case 'a':
                $unpacked[] = array($key => 'clientid:' . $id);
                break;
            case 'b':
                $unpacked[] = array($key => 'bannerid:' . $id);
                break;
            case 'p':
                $unpacked[] = array($key => 'companionid:' . $id);
                break;
        }
    }
    return $unpacked;
...
case '==': $aContext['client']['include'][$value] = true; break;
                ...
                default:
                    switch ($key) {
                        case '!=':
                            $aContext['banner']['exclude'][$value] = true;
                            break;
                        case '==':
                            $aContext['banner']['include'][$value] = true;
                            break;
                    }
            }
        }
