...
{if $order->can_pay and (empty($payment) or $payment->status lt Model_Payment::STATUS_Authorized)}
...
{elseif not empty($payment) and $payment->status gte Model_Payment::STATUS_Authorized}
...
{else}
    ...
    <p>
    <img src="/i/attention.png" align="left" />
    Пожалуйста, приготовьте Вашу пластиковую карту заранее. <b>Если Вы не&nbsp;успеете ввести данные карты за&nbsp;20 минут, то&nbsp;оплата будет заблокирована.</b>
    В&nbsp;этом случае Вы можете <a href="{Route::url('order_detail', ['id' => $order->id])}">повторить заказ</a></p>
