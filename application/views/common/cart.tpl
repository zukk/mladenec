<a id="cart" href="{Route::url('cart')}" title="{if $cart->get_qty() eq 0}Корзина пуста, добавьте товары{else}Оформить заказ{/if}">
    <span><i></i></span>
    <span>
        <small class="status">
            Cтатус: <abbr id="abbr" class="info" abbr="Условия получения скидки <b>Любимый клиент</b><ul>
        ...
        <ins id="cart-total" data-price="{$cart->get_total()}">{$cart->get_total()|price}</ins>
