...
/**
 * Класс для обмена данными с 1с, отправляет и принимает данные по спец-протоколу для обмена с 1с (см. wiki в redmine)
 * Class Odinc
 */
...
class Odinc {
    ...
    public function action_catalog_upload()
    {
        ...
        $token = Profiler::start($this->action, 'parse');
        ...
        switch($this->action) {
            ...
            case 'product':
                ...
                Profiler::stop($token);
                $stats = Profiler::group_stats($this->action);
                Log::instance()->add(Log::INFO, $this->request->action() . ($this->action ? ', action '.$this->action : '').', parse: '.$stats[$this->action]['total']['time']);

                $token = Profiler::start($this->action, 'qas');
                Model_Good::on_qas_change($good_ids);
                Profiler::stop($token);
                $stats = Profiler::group_stats($this->action);
                Log::instance()->add(Log::INFO, $this->request->action() . ($this->action ? ', action '.$this->action : '').', QAS change processing finished timer: ' . $stats[$this->action]['total']['time']);
